...
class FeatureInfo(BaseInfo):
    ...
    def __init__(self, elem):
        ...
        if self.supported:
            self.supported = self.supported.split('|')
...
#   apiname - list of strings matching <api> 'apiname' attribute, e.g. 'gl'.
...
class Registry:
    ...
    #   api - api for the element
    #   dictionary - self.{type|enum|cmd}dict
    def lookupElementInfo(self, fname, api, dictionary):
        key = (fname, api)
        if (key in dictionary):
    ...
    def markTypeRequired(self, typename, api, required):
        """Require (along with its dependencies) or remove (but not its dependencies) a type"""
        self.gen.logMsg('diag', '*** tagging type:', typename, '-> required =', required)
        # Get TypeInfo object for <type> tag corresponding to typename
        type = self.lookupElementInfo(typename, api, self.typedict)
        if (type != None):
            ...
            if (required and ('requires' in type.elem.attrib)):
                ...
                self.markTypeRequired(depType, api, required)
    ...
    # api - api for the element
    # required - boolean (to tag features as required or not)
    def markRequired(self, features, api, required):
        ...
        for cmdElem in features.findall('command'):
            ...
            cmd = self.lookupElementInfo(name, api, self.cmddict)
            if (cmd != None):
                ...
                if (required):
                    ...
                    for ptype in cmd.elem.findall('.//ptype'):
                        ...
                        self.markTypeRequired(ptype.text, api, required)
            else:
    ...
    #   api - api the feature is generated for
    #   dictionary - of *Info objects - self.{type|enum|cmd}dict
    #   genProc - bound function pointer for self.gen.gen{Type|Enum|Cmd}
    def generateFeature(self, fname, ftype, api, dictionary, genProc):
        f = self.lookupElementInfo(fname, api, dictionary)
        if (f == None):
        ...
        if (ftype == 'type'):
            if ('requires' in f.elem.attrib):
                ...
                self.gen.logMsg('diag', '*** Generating required dependent type',
                                depname)
                self.generateFeature(depname, 'type', api, self.typedict,
                                     self.gen.genType)
        elif (ftype == 'command'):
    ...
    #   api - api for the interface
    def generateRequiredInterface(self, interface, api):
        ...
        for features in interface.findall('require'):
            for t in features.findall('type'):
                self.generateFeature(t.get('name'), 'type', api, self.typedict,
                                     self.gen.genType)
            for e in features.findall('enum'):
                self.generateFeature(e.get('name'), 'enum', api, self.enumdict,
                                     self.gen.genEnum)
            for c in features.findall('command'):
                self.generateFeature(c.get('name'), 'command', api, self.cmddict,
                                     self.gen.genCmd)
    #
    ...
    def apiGen(self, genOpts):
        ...
        for key in self.apidict:
            ...
            api = fi.elem.get('api')
            if (self.genOpts.apiname and api in self.genOpts.apiname):
                apiMatch = True
        ...
        for f in features:
            ...
            self.generateRequiredInterface(f.elem, f.getApi(self.genOpts.apiname))
            self.gen.endFeature()
        self.gen.endFile()
    #
